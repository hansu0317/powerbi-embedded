...
| `docs/08_GET_필터_방식_참고.md` | GET/SDK 필터는 **진짜 RLS가 아니다** — 브라우저 개발자도구로 뚫림, 데이터 보안 경계로 쓰면 안 되는 이유 |

### 개발 — 코드를 고치거나 새 기능을 붙일 때

| 문서 | 내용 |
|---|---|
| `docs/00_학습_로드맵.md` | 처음 이 프로젝트를 볼 때 시작점 — 요청 흐름/DB 풀/인증/CORS/캐시/설정을 코드로 따라가며 학습 |
| `docs/10_프로젝트_코드구조_학습가이드.md` | 파일 지도, 읽는 순서 요약본 |
...
| `docs/09_PowerBI_Fabric_API_개발가이드.md` | Power BI REST vs Fabric REST 구분, 새 외부 API 붙일 때 체크리스트 |
